...
export interface FriendRequestDTO {
    requestId: string;          // Friendship row UUID
    user: PublicUserDTO;        // The other user (sender if incoming, receiver if outgoing)
...
export interface FriendsStatePayload {
    friends: FriendDTO[];
    pending: FriendRequestDTO[];   // direction = INCOMING
    sent: FriendRequestDTO[];      // direction = OUTGOING
}
